...
class ActorPF extends Actor {
  ...
  prepareData(actorData) {
    ...
    let saveBonus = getProperty(actorData.flags, "pathfinder.saveBonus") || 0;
    ...
    let bonusDC = getProperty(actorData.flags, "pathfinder.spellDCBonus") || 0;
  ...
  async createOwnedItem(itemData, options) {
    if (!this.isPC && itemData.type === "weapon") mergeObject(itemData, {
      "data.proficient.value": true
    });
  ...
  rollAbilitySave(abilityId, options = {}) {
    ...
    const saveBonus = this.data.flags.pathfinder && this.data.flags.pathfinder.saveBonus;
